...
//Struct for path and its evaluation
struct PathNode  
{  
	Path path;
...
//Next hop Nodes struct for Heuristic search strategy
struct NextHopNode{
	int  nodeID;
	int  cost;
	bool isEnd;
	...
	bool isGreenEdge;
	NextHop() {}
	bool operator < (const NextHopNode & hop){
...
private:
	...
	vector<vector<int>> board; 	  // adjacency matrix ( board[i][j] = the cost of edge(i, j), board[i][j] = -1 if blocked )
	vector<vector<int>> neighbor; // neighbour[i] stores neighbor nodes of node(i) 
	vector<int> constraints; 	  // store constraints, initialized to -1,  0: is a constraints; >0: has been satisfied
...
public:
	bool isHeuristic = false;//using Heuristic search or not, true: use; false: not use
	...
	int getNextHopList(vector<NextHopNode> & hopList, int st);
	//Update state
	...
	void dfs_heuristic(int st, int ed, int step, int maxHop);
