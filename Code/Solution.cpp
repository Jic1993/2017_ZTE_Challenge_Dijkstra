...
int Solution::getNextHopList(vector<NextHopNode> & hopList, int st){
	vector<int> neighborList = neighbor[st]; //get node(st) 's  neighbor list
	NextHopNode tmp;
	for(auto &next:neighborList){
...
void Solution::dfs(int st, int ed, int step, int maxHop){
	...
	int nHop = neighbor[st].size(); //# of node(st) 's neighbors
	int next; //ID of next hop node
...
//DFS with heuristic search strategy
void Solution::dfs_heuristic(int st, int ed, int step, int maxHop){
	if(!curMax.path.empty() && curCost > get<1>(curMax.evaluation)) return; //trim
	...
	for(int i = 0; i < nHop; ++i){	
		...
		update(st, next);
		dfs_heuristic(next, ed, step+1, maxHop);
		resotre(st, next);
	}			
}
...
void Solution::solve() {
	...
	curLoss = mustPass;
	if(isHeuristic){
		cout<<endl<<"---------With Heuristic search strategy---------"<<endl;
		dfs_heuristic(start, end, 1, maxHop); //call dfs_heuristic() to find solution
	}
	else{
		cout<<endl<<"---------Without Heuristic search strategy---------"<<endl;
